...
for node in $* ; do
	...
	log_file="tmp/log_${user_node}"
	echo "copy to node $node"
	rsync -arvzhe ssh $archive $user_node_dir 
	ssh $user_node tar -mxzf ${my_remote_dir}/${archive_name} -C ${my_remote_dir}
	ssh -f $user_node "cd /home/richer/export/cpp_aos_soa; app/scripts/run_all_remote.sh " >$log_file 2>&1
