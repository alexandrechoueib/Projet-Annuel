...
TEST(TestNumericBase, convert) {
	eze::integer i;
	eze::NumericBase& b_bin = eze::NumericBaseManager::instance().by_name("binary");
	eze::NumericBase& b_oct = eze::NumericBaseManager::instance().by_name("octal");
	eze::NumericBase& b_hex = eze::NumericBaseManager::instance().by_name("hexadecimal");
...
TEST(TestNumericBase, by) {
	...
	eze::NumericBase& b_oct = eze::NumericBaseManager::instance().by_suffix("123o");
	EXPECT_EQ(b_oct.name(), "octal");

	eze::NumericBase& b_hex = eze::NumericBaseManager::instance().by_suffix("abcdef_h");
	EXPECT_EQ(b_hex.name(), "hexadecimal");

	eze::NumericBase& b_dec = eze::NumericBaseManager::instance().by_suffix("1234567");
