...
#ifndef SRC_VERSION_2018_06_CUME_KERNEL_H_
#define SRC_VERSION_2018_06_CUME_KERNEL_H_
...
#define kernel_call_no_resource(name, kc, ...) \
	ez::cume::Timer kernel_timer; \
	ez::cume::Devices::get_instance().select(kc.get_device_id()); \
...
#define gtid_1_x() threadIdx.x

// grid of x blocks of 1 thread
#define gtid_x_1() blockIdx.x

// grid of x by y blocks of 1 thread
#define gtid_xy_1() (gridDim.x * blockIdx.y) + blockIdx.x

// grid of 1 block of x by y threads
#define gtid_1_xy() blockDim.x * threadIdx.y + threadIdx.x

// grid of x block of x' threads
#define gtid_x_x() blockDim.x * blockIdx.x + threadIdx.x

// grid of x by y blocks of x' threads
#define gtid_xy_x() (gridDim.x * blockDim.x) * blockIdx.y + blockIdx.x * blockDim.x + threadIdx.x

// grid of y blocks of x by y' threads
#define gtid_x_xy() blockIdx.x * (blockDim.x * blockDim.y) + threadIdx.y * blockDim.x + threadIdx.x

// grid of x by y blocks of x' by y' threads
#define gtid_xy_xy() blockIdx.y * (blockDim.x * blockDim.y) * gridDim.x \
						+ blockIdx.x * (blockDim.x * blockDim.y) \
						+ threadIdx.y * blockDim.x + threadIdx.x

// grid of x by y by z blocks of 1 thread
#define gtid_xyz_1() blockIdx.z * (gridDim.x * gridDim.y) \
...
public:
	...
	typedef struct Coordinate {
		integer x, y, offset;
		__device__ Coordinate(integer _x, integer _y, integer cols)  :x(_x), y(_y)  {
			offset = _y * cols + _x;
		}
	} Coordinate;
	...
	struct Resource {
		 ...
		 */
		__device__ integer get_global_tid() {
			switch(kernel_type) {
		 ...
		 */
		__device__ integer get_local_tid() {
			switch(kernel_type) {
				case KERNEL_TYPE_NONE:
					return 0;
...
protected:
	...
	natural required_threads;
	// device id on which the kernel will be run
	integer device_id;
	// is timer needed ?
	boolean timer_needed;
...
public:
	 ...
	 */
	Kernel(natural nbt, integer dev_id = Devices::DEVICE_0);
	 ...
	 */
	void configure(integer gmod, integer bmod, ...) ;

	/**
	 * return identifier of device on which computations
	 * are performed
	 */
	integer get_device_id() {
		return device_id;
	}

	void set_timer_needed(boolean value=true) {
		timer_needed = value;
	}

	boolean is_timer_needed() {
		return timer_needed;
	}
...
#endif /* SRC_VERSION_2018_06_CUME_KERNEL_H_ */
