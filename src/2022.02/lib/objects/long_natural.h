...
*/
#ifndef OBJECTS_LONG_NATURAL_H_
#define OBJECTS_LONG_NATURAL_H_
 ...
 */
class LongNatural : public Object {
public:
	typedef LongNatural self;
	 ...
	 */
	static LongNatural zero_object;


	/**
	 * default constructor
	 */
	LongNatural() : Object(), m_value(0) {
	 ...
	 */
	LongNatural(natural x) : Object(), m_value(x) {
	 ...
	 * copy constructor
	 */
	LongNatural(const LongNatural& obj) : Object() {
		m_value = obj.m_value;
	}
	 ...
	 * assignment operator
	 */
	LongNatural& operator=(const LongNatural& obj) {
		if (&obj != this) {
			m_value = obj.m_value;
		}
		return *this;
	...
	Object *clone() {
		return new LongNatural(m_value);
	}
	...
	integer compare(const Object& y) {
		LongNatural& y_obj = *dynamic_cast<LongNatural *>(&const_cast<Object&>(y));
	 ...
	 */
	static long_natural cnp(int n, int p);

	/**
	 * return A(n,p) = n! / (n-p)!
	 */
	static long_natural anp(int n, int p);
	 ...
	 */
	natural factorial() {
		return LongNatural::factorial(m_value);
	}

	natural fibonacci() {
		return LongNatural::fibonacci(m_value);
	}

	natural sqrt() {
		return LongNatural::sqrt(m_value);
	}

	bool is_prime() {
		return LongNatural::is_prime(m_value);
	}
